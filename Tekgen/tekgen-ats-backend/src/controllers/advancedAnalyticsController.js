...
class AdvancedAnalyticsController {
  ...
  async listInterviews(req, res) {
    try {
      ...
      const data = await interviewScheduleService.list({
        candidateId: candidateId || undefined,
      });
      sendSuccess(res, data, 'Interviews retrieved');
    } catch (error) {
  ...
  async deleteInterview(req, res) {
    try {
      ...
      sendSuccess(res, { id: req.params.id }, 'Interview deleted');
    } catch (error) {
      logger.error('Delete interview error', error);
      sendError(res, error.message || 'Failed to delete interview', 500);
    }
  }
  ...
  async scheduleInterviewWithTracking(req, res) {
    try {
      const b = req.body || {};
      const titleRaw = b.jobTitle != null ? String(b.jobTitle).trim() : '';
      let interviewDate = b.interviewDate;
      let time = b.time;
      if ((!interviewDate || !time) && b.dateTime) {
        const dt = new Date(b.dateTime);
        if (!Number.isNaN(dt.getTime())) {
          interviewDate = dt.toISOString().slice(0, 10);
      ...
      const payload = {
        ...
        jobTitle: titleRaw || 'Interview discussion',
        interviewDate,
        time,
        interviewType: b.interviewType || b.type || 'video',
        location: b.location || null,
        meetLink: b.meetLink || null,
        notes: b.notes || null,
        sendCalendarInvite: b.sendCalendarInvite !== false,
        notifyTeamsChannel: b.notifyTeamsChannel !== false,
        sendWhatsApp: b.sendWhatsApp === true,
        durationMinutes: b.durationMinutes || 60,
      };

      if (!payload.candidateEmail || !payload.candidateName) {
        return sendError(res, 'candidateEmail and candidateName are required', 400);
      }
      if (!payload.interviewDate || !payload.time) {
        return sendError(res, 'interviewDate and time (or dateTime) are required', 400);
      }

      const result = await interviewScheduleService.scheduleFromForm(payload, req.user.id);
