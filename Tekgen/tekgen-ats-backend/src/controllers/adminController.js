...
class AdminController {
  ...
  /**
   * Force selected users to change password on next login (ADMIN only)
   */
  async forcePasswordChange(req, res) {
    try {
      ...
      const result = await prisma.user.updateMany({
        ...
        data: { lastLoginAt: null, loginAttempts: 0, isAccountLocked: false },
      });

      logger.info(`Admin forced password change for ${result.count} users`);
      return sendSuccess(
        res,
        { updated: result.count, requested: uniqueIds.length },
        'Password change enforced for selected users'
      );
    } catch (error) {
      logger.error('Admin forcePasswordChange error', error);
      return sendError(res, 'Failed to enforce password change', 500);
    }
  }
