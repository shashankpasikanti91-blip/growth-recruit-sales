...
function CommCard({ platform, info, integration, isAdmin, onSaved, onDeleted }) {
  ...
  const isVerified   = integration?.isVerified;
    ...
    <div className={`bg-white rounded-xl border-2 ${info.border} overflow-hidden shadow-sm`}>
      ...
      <div className={`flex items-center justify-between px-5 py-4 ${info.bg} border-b ${info.border}`}>
        ...
        <span className={`ml-2 flex-shrink-0 flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${
          isConfigured && isVerified && integration?.isEnabled ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
        }`}>
          {isConfigured && isVerified && integration?.isEnabled
            ? <><Wifi size={9} /> Verified</>
            : <><WifiOff size={9} /> {isConfigured ? 'Saved (test pending)' : 'Not Set'}</>}
        </span>
      </div>
        ...
        {isConfigured && (!isFromGlobal || isAdmin) && !editing && (
          <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-4 py-3">
            <div className="text-sm text-green-800 font-semibold flex items-center gap-2">
              ...
              {!isVerified && <span className="text-xs text-amber-700">(run Test to verify)</span>}
          ...
          <div className="p-6 space-y-8">
            ...
            <section className="border-t border-gray-200 pt-6">
              <h3 className="text-sm font-bold text-gray-600 mb-3 flex items-center gap-2">
                <Info size={13} /> After Connection - Next Steps
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-600">
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  <p className="font-semibold mb-1">Outlook + Teams Calendar</p>
                  <p>1) Save credentials 2) click Test until status is Verified 3) use <strong>Interviews</strong> and <strong>Emails</strong> pages to schedule and send communication.</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  <p className="font-semibold mb-1">WhatsApp + Telegram</p>
                  <p>Save token, run Test (must become Verified), then use candidate workflows to send messages. Invalid tokens stay in Saved state only.</p>
                </div>
              </div>
            </section>
