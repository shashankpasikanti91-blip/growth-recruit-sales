...
function InfoCard({ label, value, mono = false, icon }) {
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
      ...
      <p className={`text-gray-900 font-semibold text-sm ${mono ? 'font-mono text-xs break-all' : ''}`}>{value || '—'}</p>
    </div>
  );
}
...
function JobPipelineRow({ row, canEdit, onReload }) {
  const [stage, setStage] = useState(row.stage);
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState('');
  const candName = [row.candidate?.firstName, row.candidate?.lastName].filter(Boolean).join(' ') || '—';

  useEffect(() => setStage(row.stage), [row.id, row.stage]);
  ...
  const apply = async (next) => {
    ...
    setErr('');
    try {
      await api.patch(`/api/submissions/${row.id}/stage`, { stage: next });
      setStage(next);
      onReload();
    } catch (e) {
  ...
  return (
    <tr className="border-t border-gray-100">
      <td className="py-2 pr-4 text-sm text-gray-900">{candName}</td>
      <td className="py-2 pr-4 text-sm text-gray-600">{row.client?.clientName || '—'}</td>
        ...
        {canEdit ? (
          <div className="space-y-1">
            <select
              ...
              className="text-xs border border-gray-300 rounded-lg px-2 py-1 bg-white max-w-[220px]"
            >
          ...
          <span className="text-xs text-gray-700">{String(stage).replace(/_/g, ' ')}</span>
        )}
      </td>
...
export default function JobViewPage() {
  ...
  const { canManageClientJobs, canUpdateSubmissionPipeline } = useRole();
  ...
  const loadSubmissions = async (jobId) => {
    ...
    try {
      ...
      const res = await api.get(`/api/submissions?jobId=${jobId}`);
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <InfoCard label="Tekgen Job ID" value={job.displayId} mono icon={<Briefcase size={12} />} />
              <InfoCard label="Client JR No" value={job.clientJrNumber || '—'} mono icon={<Tag size={12} />} />
              <InfoCard
                label="Client Hiring Request ID"
                value={job.clientRequestUuid || '—'}
                mono
                icon={<Tag size={12} />}
              />
            ...
            {/* Client submission pipeline (manual stage: sales or assigned recruiters) */}
            <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-5">
              <h3 className="text-sm font-bold text-gray-900 mb-1 flex items-center gap-2">
                <Users size={15} className="text-violet-600" /> Candidate pipeline (client submissions)
              </h3>
              <p className="text-xs text-gray-500 mb-4">
                Update stage when profiles move: internal → sales → client → interview → offer / joined / rejected.
                Assignees get in-app notifications on stage changes (for handoffs if someone was busy).
              </p>
              {submissionsLoading ? (
                <p className="text-sm text-gray-400">Loading submissions…</p>
              ) : submissions.length === 0 ? (
                <p className="text-sm text-gray-400 italic">No submissions linked to this JD yet.</p>
              ) : (
