  ...
  const fetchCandidates = useCallback(async () => {
    try {
      ...
      const r = await apiClient.get('/api/candidates', {
        params: { status: 'HIRED', search: search || undefined, limit: PAGE_LIMIT, page },
      });
        ...
        <div className="flex-1 overflow-auto" style={{ minWidth: 0 }}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-60 gap-3">
              ...
              <p className="text-slate-400 text-xs">Only candidates with status <strong>HIRED</strong> appear here</p>
            </div>
          ) : (
