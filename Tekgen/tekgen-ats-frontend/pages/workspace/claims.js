...
export default function MyClaims() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/api/my/claims', fd);
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div className="col-span-2 md:col-span-3">
                  <label className="block text-xs font-medium text-slate-500 mb-1">Proof Attachments (receipt/invoice, multiple)</label>
                  <input
                    type="file"
                    multiple
                  ...
                  {attachments.length > 0 && (
                    <p className="text-xs text-slate-500 mt-1">{attachments.length} file(s) selected</p>
                  )}
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                        ...
                        <th className="px-4 py-2">Proof Files</th>
                      </tr>
                    </thead>
                              ...
                              <div className="flex flex-col gap-1">
                                {c.attachments.map((a) => (
                                  <button
                                    key={a.id}
                                    type="button"
                                    onClick={() => openAuthenticatedFile(`/api/secure-files/attachments/${a.id}/download`)}
