...
export default function Topbar({ title }) {
  ...
  const getBreadcrumb = () => {
    ...
    const labelMap = {
      ...
      hr: 'HR Operations', payroll: 'Payroll', visa: 'HR Ops Visa & Permits',
  ...
  return (
    <header className="h-14 bg-white border-b border-slate-200 flex items-center px-5 gap-4 sticky top-0 z-30 shadow-sm flex-shrink-0">
      ...
      <div className="relative" ref={userRef}>
        ...
        >
          {user?.avatar ? (
            <img src={user.avatar} alt="User avatar" className="w-7 h-7 rounded-full object-cover border border-slate-200 flex-shrink-0" />
          ) : (
