...
| Role | Access |
|------|--------|
| Finance Manager | Full invoice management, payments, collections, reporting |
| Assistant MD | Approval/oversight for commercial controls |
| Managing Director | Full access |
| Management | View reports and dashboards (if assigned) |
| Sales Executive | View invoice status + collection follow-up for own clients only |
| Payroll Manager | No invoice access (unless also has Finance role) |
...
- Finance backend routes are present for dashboard, invoices, agreements, payments, and collections.
- Finance role families (`FINANCE`, `FINANCE_HEAD`) are already wired in backend authorization for finance modules.
- Quick login now includes Finance Manager demo account for validation and testing.
- Remaining work: unify UI role labels to “Finance Manager”, tighten row-level client ownership controls, and complete finance dashboards/reports polishing.
