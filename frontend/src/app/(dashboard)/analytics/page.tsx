...
const STAGE_COLORS: Record<string, string> = {
  ...
  INTERVIEW: '#10b981', OFFER: '#06b6d4', HIRED: '#22c55e', REJECTED: '#ef4444',
};

export default function AnalyticsPage() {
  const [days, setDays] = useState(30);

  const { data: recruitment } = useQuery({ queryKey: ['analytics', 'recruitment', days], queryFn: () => analyticsApi.recruitment(days) });
  const { data: sales } = useQuery({ queryKey: ['analytics', 'sales', days], queryFn: () => analyticsApi.sales(days) });
  const { data: aiUsage } = useQuery({ queryKey: ['analytics', 'ai-usage', days], queryFn: () => analyticsApi.aiUsage(days) });

  const stageData = recruitment?.applications?.byStage?.map((s: any) => ({ name: s.stage, count: s._count, fill: STAGE_COLORS[s.stage] ?? '#6b7280' })) ?? [];
  const leadData = sales?.leads?.byStage?.map((s: any) => ({ name: s.stage, count: s._count })) ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'New Candidates', value: recruitment?.candidates?.total ?? 0, color: 'text-blue-600' },
          { label: 'Applications', value: recruitment?.applications?.total ?? 0, color: 'text-purple-600' },
          { label: 'New Leads', value: sales?.leads?.total ?? 0, color: 'text-amber-600' },
          { label: 'AI Cost (USD)', value: `$${totalAiCost.toFixed(4)}`, color: 'text-green-600' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-4">Recruitment Funnel</h2>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={stageData} margin={{ left: -20 }}>
              ...
              <Bar dataKey="count" radius={[4,4,0,0]}>
                {stageData.map((e: any, i: number) => <Cell key={i} fill={e.fill} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-4">Sales Pipeline Stages</h2>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={leadData} margin={{ left: -20 }}>
              <XAxis dataKey="name" tick={{ fontSize: 11 }} />
              <YAxis tick={{ fontSize: 11 }} />
              <Tooltip />
      ...
      {aiUsage?.usage?.length > 0 && (
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-4">AI Service Usage</h2>
          <div className="overflow-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  {['Service', 'Provider', 'Calls', 'Tokens', 'Cost (USD)'].map(h => (
                    <th key={h} className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {aiUsage.usage.map((u: any, i: number) => (
                  <tr key={i}>
                    <td className="px-4 py-3 text-gray-900">{u.serviceType}</td>
                    <td className="px-4 py-3 text-gray-500">{u.provider}</td>
                    <td className="px-4 py-3 font-medium">{u._count}</td>
                    <td className="px-4 py-3 text-gray-500">{u._sum?.tokensUsed?.toLocaleString() ?? 0}</td>
                    <td className="px-4 py-3 text-green-600 font-medium">${(u._sum?.costUsd ?? 0).toFixed(4)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {recruitment?.topSkills?.length > 0 && (
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-4">Top Skills in Talent Pool</h2>
          <div className="flex flex-wrap gap-2">
            {recruitment.topSkills.map(({ skill, count }: { skill: string; count: number }) => (
              <span key={skill} className="badge-blue">
                {skill} <span className="ml-1 font-semibold">{count}</span>
              </span>
            ))}
          </div>
        </div>
