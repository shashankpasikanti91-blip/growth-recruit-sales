...
export default function AnalyticsPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await apiClient.get('/api/analytics/timeline').catch(() => null);
  ...
  const timeline = data.past ?? [];

  return (
    <DashboardLayout title="Analytics & Performance">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Track team productivity, pipeline health, and hiring velocity</p>
          </div>
        </div>

        {/* KPI cards */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard label="Total Candidates"   value={data.totals?.totalCandidates}   sub="in pipeline"    icon={Users}        iconBg="bg-brand-500"   trend={12} />
          <KpiCard label="Active JDs"         value={data.kpis?.activeJDs}           sub="currently open" icon={Briefcase}    iconBg="bg-violet-500"  trend={-5} />
          <KpiCard label="Hired This Quarter" value={data.totals?.totalHired}         sub="offer accepted" icon={CheckCircle2} iconBg="bg-emerald-500" trend={8} />
          <KpiCard label="Avg Time to Hire"   value={`${data.kpis?.avgTimeToHire}d`} sub="target: 30d"    icon={Clock}        iconBg="bg-amber-500"   trend={-3} />
        </div>
        ...
        {tab === 'recruiters' && (
          <div className="space-y-5">
            ...
            <div className="card p-5">
              <h3 className="section-title mb-4">Submission Comparison</h3>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={data.recruiters} barGap={6} barSize={22}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
            ...
            <div className="card overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {data.recruiters?.map((r, i) => (
                      <RecruiterRow key={r.name} recruiter={r} index={i} maxSubmissions={maxSubmissions} />
                    ))}
                  </tbody>
                </table>
              </div>
