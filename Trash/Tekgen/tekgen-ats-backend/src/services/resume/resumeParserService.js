...
class ResumeParserService {
  ...
  extractContactInfo(text) {
    ...
    // Helper: clean up a matched phone string
    const cleanPhone = (s) => s.trim().replace(/[\s]{2,}/g, ' ');

    // 1) Labeled phone fields — most reliable signal
    //    Phone: / Mobile: / Tel: / HP: / H/P: / Handphone: / Contact: / WhatsApp:
    const labeledMatch = text.match(
      /(?:phone|mobile|tel(?:ephone)?|h\/p|hp|handphone|cell(?:phone)?|contact\s*no\.?|whatsapp|fax)\s*(?:no\.?|number|num|#)?\s*[:\-–]?\s*([\+\d][\d\s\-\.\(\)]{6,22})/i
    );
    if (labeledMatch) {
      phone = cleanPhone(labeledMatch[1]);
    }

    // 2) International prefix: +60 (MY), +91 (IN), +62 (ID), +65 (SG),
    //    +63 (PH), +94 (LK), +880 (BD), +92 (PK), +66 (TH), +84 (VN),
    //    +95 (MM), +977 (NP), +960 (MV), and others
    if (!phone) {
      const intlMatch = text.match(
        /\+(?:880|960|977|\d{1,3})[\s\-]?\(?\d{1,4}\)?[\s\-]?\d{2,4}[\s\-]?\d{3,5}[\s\-]?\d{0,5}/
      );
      if (intlMatch) {
        const candidate = intlMatch[0].replace(/[^\d\+\s\-]/g, '').trim();
        // must have at least 7 digits total
        if ((candidate.match(/\d/g) || []).length >= 7) {
          phone = cleanPhone(intlMatch[0]);
        }
      }
    }

    // 3) Malaysia local format (without +)
    //    01X-XXXXXXX, 011-XXXXXXXX, 0X-XXXXXXX (landline)
    //    Handles: 0122824566 / 012-282 4566 / 012 282 4566 / 012.282.4566
    if (!phone) {
      const myLocal = text.match(
        /\b0(?:1[0-46-9])[\s\-\.]?\d{3,4}[\s\-\.]?\d{4}\b|\b011[\s\-\.]?\d{4}[\s\-\.]?\d{4}\b|\b0[2-9][\s\-\.]?\d{3,4}[\s\-\.]?\d{4}\b/
      );
      if (myLocal) phone = cleanPhone(myLocal[0]);
    }

    // 4) Malaysia: 60XXXXXXXXXX or 60 1X XXX XXXX (country code without +)
    //    Avoid matching year-like numbers by requiring 10+ digits total
    if (!phone) {
      const my60 = text.match(
        /\b60[\s\-]?1[0-46-9][\s\-]?\d{3,4}[\s\-]?\d{4}\b|\b60[\s\-]?11[\s\-]?\d{4}[\s\-]?\d{4}\b/
      );
      if (my60) phone = '+' + cleanPhone(my60[0]);
    }

    // 5) India local: 10-digit starting with 6,7,8,9
    if (!phone) {
      const inLocal = text.match(/\b[6-9]\d{4}[\s\-]?\d{5}\b/);
      if (inLocal && (inLocal[0].match(/\d/g) || []).length === 10) {
        phone = cleanPhone(inLocal[0]);
      }
    }

    // 6) Generic fallback: +country_code then digits (already has + but didn't match above)
    if (!phone) {
      const generic = text.match(/\+\d{1,3}[\s\-]?\d{2,4}[\s\-]?\d{3,4}[\s\-]?\d{3,5}/);
      if (generic && (generic[0].match(/\d/g) || []).length >= 8) {
        phone = cleanPhone(generic[0]);
      }
    }
