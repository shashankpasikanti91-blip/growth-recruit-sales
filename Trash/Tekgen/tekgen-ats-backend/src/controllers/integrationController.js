...
class IntegrationController {
  ...
  async saveIntegration(req, res) {
    try {
      ...
      }, `${platform} configured${makeGlobal ? ' — system-wide for all users' : ''}`);
    } catch (error) {
  ...
  async testIntegration(req, res) {
    try {
      ...
      sendSuccess(res, { platform, status: 'configured', message: `${platform} API key is set` }, 'Integration configured');
    } catch (error) {
