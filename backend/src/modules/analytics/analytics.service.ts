...
export class AnalyticsService {
  ...
  async getDashboardKpis(tenantId: string) {
    ...
    const [totalCandidates, totalApplications, stageBreakdown, aiScreened, topSkills] =
      await Promise.all([
        this.prisma.candidate.count({ where: { tenantId, createdAt: { gte: since } } }),
        this.prisma.application.count({ where: { tenantId, appliedAt: { gte: since } } }),
        ...
        this.prisma.aiAnalysisResult.count({
          where: { tenantId, serviceType: 'RESUME_SCREENER', createdAt: { gte: since } },
        }),
    ...
    const topSkillList = Object.entries(skillCountMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([skill, count]) => ({ skill, count }));
    ...
    return {
      ...
      applications: { total: totalApplications, byStage: stageBreakdown },
      aiScreenings: { total: aiScreened },
      topSkills: topSkillList,
    };
  }
  ...
  async getSalesSummary(tenantId: string, days = 30) {
    ...
    const [totalLeads, stageBreakdown, totalCompanies] = await Promise.all([
      this.prisma.lead.count({ where: { tenantId, createdAt: { gte: since } } }),
      this.prisma.lead.groupBy({ by: ['stage'], where: { tenantId }, _count: true }),
      this.prisma.company.count({ where: { tenantId } }),
    ]);
    ...
    return {
      period: { days, since },
      leads: { total: totalLeads, byStage: stageBreakdown },
